...
export const ErrorMessage: React.FC<ErrorMessageProps> = ({ error, onClose, onRetry }) => {
  ...
  const isMixedContentError = error.includes('insecure connection') ||
                             error.includes('cannot be played on this secure site');
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium mb-1">
            {isMixedContentError ? 'Security Restriction' : 
             isConnectionError ? 'Connection Issue' : 'Station Error'}
          </p>
          <p className="text-xs text-red-100 break-words">
            {error}
          </p>
          {isMixedContentError && isSecureSite && (
            <p className="text-xs text-red-200 mt-2">
              🔒 This secure site can only play stations with HTTPS streams. The app will automatically try HTTPS-compatible stations.
            </p>
          )}
          {isConnectionError && (
            <p className="text-xs text-red-200 mt-2">
